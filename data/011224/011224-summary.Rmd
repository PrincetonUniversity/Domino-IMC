---
title: "WebRTC Cellular 01/12/24 Experiment: Summary"
date: "`r Sys.Date()`"
output:
  html_document:
    df_print: paged
...
```{r}
streams_ue %>%
  mutate(dur_s = as.integer((max_ts_ms - min_ts_ms) / 1000)) %>%
  mutate(type = as.factor(case_when(
    dir == "up"   & rtp_pt == 96  ~ "video_main",
    dir == "up"   & rtp_pt == 97  ~ "video_rtx",
    dir == "up"   & rtp_pt == 111 ~ "audio",
    dir == "down" & rtp_pt == 100 ~ "audio",
    dir == "down" & rtp_pt == 101 ~ "video_main",
    dir == "down" & rtp_pt == 102 ~ "video_rtx"
  ))) %>%
  select(-c(min_ts_ms, max_ts_ms)) %>%
  arrange(dir, -n)
```
...
pkts_ue %>%
  ...
  ggplot(aes(x = ts_s, y = pkts, color = flow_id)) +
    labs(title = "Packets Received/Sent by IP-5T Flow", x = "Time [s]", y = "Packets/s",
         color = "Flow") +
    geom_line() +
...
pkts_ue %>%
  ...
  ggplot(aes(x = ts_s, y = bytes, color = flow_id)) +
    labs(title = "Data Rate Received/Sent by IP-5T Flow",x = "Time [s]", y = "Data Rate [KBit/s]", 
         color = "Flow") +
    geom_line() +
    facet_wrap(~dir,  ncol=1) +
    theme_om() +
