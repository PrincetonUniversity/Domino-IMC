---
title: "Zoom 06/24 5G - Join"
date: "`r Sys.Date()`"
output:
  html_document:
    df_print: paged
params:
  experiment_name: "zoom-0624-5g"
---
...
```{r sender-ping-ts1, fig.height = 4, fig.width = 8}
sender_delay %>%
...
```
```{r sender-ping-ts2, fig.height = 6, fig.width = 10, warning = FALSE}
sender_delay %>%
  pivot_longer(c("sender_core_ms", "core_sender_ms", "rtt_ms"), names_to="leg", values_to="val") %>%
  ggplot(aes(x = idx, y = val, color = leg)) +
    labs(title = "Sender ICMP Delay", x = "Packet Index", y = "Latency [ms]") +
...
```{r receiver-ping-ts1, fig.height = 4, fig.width = 8}
receiver_delay %>%
  slice(2:670) %>%
...
```{r receiver-ping-ts2, fig.height = 4, fig.width = 8}
receiver_delay %>%
  ggplot(aes(x = idx, y = rtt_ms)) +
    labs(title = "Receiver ICMP Delay", x = "Packet Index", y = "Latency [ms]") +
    ylim(0, 40) +
...
```{r receiver-ping-cdf, fig.height = 4, fig.width = 6, warning = FALSE}
receiver_delay %>%
  ggplot(aes(x = rtt_ms)) +
