---
title: "WebRTC Cellular 02/27 ICMP Echo Experiment: One-way Delay"
date: "`r Sys.Date()`"
...
pkts_ue_up <- pkts_ue %>%
  filter(ip_src == "192.168.0.30" & ip_dst == "192.168.68.8")
pkts_ue_down <- pkts_ue %>%
  filter(ip_dst == "192.168.0.30" & ip_src == "192.168.68.8")
...
```{r results='asis'}
bind_rows(
  pkts_ue %>% filter(icmp_id == "6d38" & icmp_seq == 8005) %>% mutate(loc = "ue"),
  pkts_enb %>% filter(icmp_id == "6d38" & icmp_seq == 8005) %>% mutate(loc = "enb"),
...
owd_up %>%
  filter(!is.na(owd_ue_enb)) %>%
  filter(icmp_seq >= 22800 & icmp_seq < 22900) %>%
  ggplot(aes(x = icmp_seq, y = owd_ue_enb)) +
    labs(x = "ICMP seq.", y = "UE-eNB one-way delay [ms]") +
    scale_x_continuous(expand = c(0.0125, 0)) +
    ylim(10, NA) +
