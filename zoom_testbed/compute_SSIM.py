...
def detect_and_crop_frame(frame, crop_fraction_width, crop_fraction_height, dark_threshold):
    ...
    target_w = int(1920 * (1 - crop_fraction_width * 2))
    target_h = int(1080 * (1 - crop_fraction_height * 2))
...
# using pylibdmtx to decode barcode
...
def process_receiver_video(video_path, original_frames, original_barcode_values, output_dir='barcode_debug_images', num_debug_images=10):
    ...
    while cap.isOpened():
        ...
        # comment out, if we want to disable the save barcode image function
        ...
        if barcode_value in original_barcode_values:
            ...
            # Print the size of both images
            # print(f"frame_index: {frame_idx}, Size of receiver frame: {cropped_frame.shape}, Size of original frame: {original_frame.shape}")

            # Compute SSIM
            if original_idx == previous_original_idx:
                ssim_value = previous_ssim_value
            else:
                ssim_value = calculate_ssim_opencv(cropped_frame, original_frame)
                previous_ssim_value = ssim_value
...
def main(input_video, output_csv):
    ...
    original_frames, original_barcode_values = process_original_video('./raw_video/Zoom1_1080p_barcode.mp4', barcode_df)
