...
def terminate_processes():
    ...
    # Clean up v4l2loopback - Note: Using sudo without password on GCP
    try:
...
def run_gcp_webrtc_commands(file_number, duration):
    try:
        ...
        # Start virtual camera (using sudo without password on GCP)
        print("[INFO] Starting virtual camera...")
        camera_cmd = ("sudo bash -c '"
        ...
        # Start tcpdump (using sudo without password on GCP)
        print("[INFO] Starting tcpdump...")
        tcpdump_cmd = (f"cd /home/fanyi/proj_webrtc/data_pcap && "
                      f"sudo tcpdump -i ens4 -G {duration + 5} -W 1 -v -w "
                      f"webrtc-{file_number_str}-gcp.pcap")
...
def main():
    parser = argparse.ArgumentParser(description='Run Google Cloud WebRTC commands')
    ...
    run_gcp_webrtc_commands(args.f, args.t)
