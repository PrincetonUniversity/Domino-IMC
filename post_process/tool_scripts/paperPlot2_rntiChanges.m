...
% 1. Uplink average PRB allocation
ax1 = subplot(9, 1, 1);
plotPrbAllocation(ulData, bin_sz_phy, slot_duration, duplex_mode, false, export_datapath);
title('Uplink Average PRB Allocation');

% 2. Uplink TBS by RNTI
ax2 = subplot(9, 1, 2);
plotTbsByRnti(ulData, bin_sz_phy, RNTIs_of_interest, false, export_datapath);
title('Uplink TBS by RNTI (UEs of Interest)');

% 3. Uplink TBS vs Packet data vs GCC target bitrate (UEs of interest only, UE-based)
ax3 = subplot(9, 1, 3);
plotCapacityComparison(ulData, bin_sz_phy, true, 'ue', false, export_datapath);  % true = UEs of interest only, ue-based
title('Uplink Capacity: TBS vs Packet vs Target Bitrate (UE-based, UEs of Interest Only)');

% 4. Uplink packet delay (UE-based)
ax4 = subplot(9, 1, 4);
plotPacketDelay(ulData, window_size, 'ue', false, export_datapath);
% Title is set within the plotPacketDelay function

% 5. Uplink GCC modified trend vs adaptive threshold
ax5 = subplot(9, 1, 5);
plotGccTrendVsThreshold(ulData, false, export_datapath);
title('Uplink Modified Trend vs. Adaptive Threshold');

% 6. Uplink GCC bandwidth state
ax6 = subplot(9, 1, 6);
plotGccBandwidthState(ulData, false, export_datapath);
title('Uplink Bandwidth State');

% 7. Uplink Outstanding Bytes
ax7 = subplot(9, 1,7);
plotGccOutstandingBytes(ulData, false, export_datapath);
title('Uplink Outstanding Bytes');

% 8. Uplink GCC metrics
ax8 = subplot(9, 1, 8);
plotGccTargetRates(ulData, false, export_datapath);
title('Uplink GCC Bitrate Estimates');

% 9. Uplink outbound resolution and framerate
ax9 = subplot(9, 1, 9);
...
% Add overall title
sgtitle('WebRTC RNTI Changes Analysis', 'FontSize', 14);