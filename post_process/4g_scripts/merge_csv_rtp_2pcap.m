...
sink_filename = ['../data/data_exp' expCode '/sink_rtp.csv'];
packets_sfu = readmatrix(sink_filename, 'Range', 2);

ue_filename = ['../data/data_exp' expCode '/ue_rtp.csv'];
packets_ue = readmatrix(ue_filename, 'Range', 2);

enb_filename = ['../data/data_exp' expCode '/sink_rtp.csv'];
    ...
    for j = 1:size(packets_sfu_ul, 1)
        if ~isnan(packets_sfu_ul(j, 14)) % Column 14 is RTP sequence number
            ...
            if ueIdx ~= -1
                ...
                packets_merge = [packets_merge; [packets_ue_data, rtp_seq, ue_ts, sfu_ts, sfu_ts, sfu_ts - ue_ts]];
            end
        end
    end
end
...
function [idx, ts, packets_ue_data] = findMatchingPacket(packets, rtp_seq, startIdx, offset)
    ...
    for i = startIdx:size(packets, 1)
        if packets(i, 14) == rtp_seq
            ...
            packets_ue_data = packets(i, 15:20); % Extracting relevant data for merging
            break;
        end
    end
end
