...
ylabel({'Media One-Way'; 'Delay (ms)'}, 'FontSize', 12);
ylim(ylim_values);
yticks([0:150:300]);
xlim(xlim_values);
grid on;
set(gca, 'XTickLabel', [], 'FontSize', 12);  % Remove x-tick labels and set font size
hold off;
...
ylabel({'RTCP One-Way'; 'Delay (ms)'}, 'FontSize', 12);
ylim(ylim_values);
yticks([0:150:300]);
xlim(xlim_values);
grid on;
...
if ~isempty(data.gcc_data_filtered)
    ...
    if ~isempty(network_data)
        ...
        if ismember('outstanding_bytes', network_data.Properties.VariableNames)
            ...
            ylabel({'Outstanding'; 'Bytes (KB)'}, 'FontSize', 12);
            ylim([0, 360]);
            yticks([0:100:300]);
        else
            text(mean(xlim_values), mean([0, 350]), 'No outstanding bytes data available', 'HorizontalAlignment', 'center', 'FontSize', 12);
        end
        ...
        if ismember('data_window_bytes', network_data.Properties.VariableNames)
            ...
            % ylabel({'Window'; 'Size (KB)'}, 'FontSize', 12);
            ylim([0, 360]);
            yticks([0:100:300]);
        else
            text(mean(xlim_values), mean([0, 350]), 'No data window size available', 'HorizontalAlignment', 'center', 'FontSize', 12);
        end
        
    else
        text(mean(xlim_values), mean([0, 350]), 'No network_controller data available', 'HorizontalAlignment', 'center', 'FontSize', 12);
    end
else
    text(mean(xlim_values), mean([0, 350]), 'GCC data not available', 'HorizontalAlignment', 'center', 'FontSize', 12);
end
...
ylabel({'GCC Rate'; '(Mbps)'}, 'FontSize', 12);
% Set y-axis limits
ylim(ylim_values);
yticks([0:1.5:4.5]);
xlim(xlim_values);
...
set(lgd, 'FontSize', 12); % Set font size to 20 and remove box
...
ylabel({'Outbound FPS'}, 'FontSize', 12);
ylim([15, 35]);
yticks(20:5:30);
...
xlabel('Timestamp at UE (s)', 'FontSize', 12);
