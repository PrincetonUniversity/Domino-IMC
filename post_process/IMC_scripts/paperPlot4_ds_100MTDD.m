...
while i <= length(ts_valid)
    ...
    while j <= length(ts_valid) && (ts_valid(j) - current_ts) <= 1
        ...
        current_ts = ts_valid(j); % Use the later timestamp
        j = j + 1;
    end
...
ts_requested = [21.9297; ts_combined];
tbs_requested = [7.128; tbs_combined];
...
for i = 1:num_packets
    ...
    if strcmpi(data.direction, 'UL')
        ...
        temp_idx_phy = find(ts_requested < temp_ts_enb, 1, 'last');
    else
        temp_ts_enb = ts_ue(i);
        temp_idx_phy = find(ts_requested < temp_ts_enb, 1, 'last');
    end
    ...
    if ~isempty(temp_idx_phy)
        ts_phy_pkt(i) = ts_requested(temp_idx_phy);
        tbs_phy_pkt(i) = tbs_requested(temp_idx_phy);
    end
end
...
h_requested = bar(ts_requested, tbs_requested, 'FaceColor', [0.4660 0.6740 0.1880], 'EdgeColor', [0.4660 0.6740 0.1880], 'LineWidth', 2, 'BarWidth', 0.6);
