...
enb2sfu_delay = 0; % ms
relative_plot_period = [132174, 132800];
skip_init_tbs = 2;
...
ts_send = data.ts_server - data.min_time;
ts_receive = data.ts_ue - data.min_time;
pkt_size = data.pkt_size;

% Create packet data for horizontal lines
num_packets = length(ts_send);
...
for i = 1:num_packets
    ...
    temp_ts_enb = ts_receive(i) - enb2sfu_delay;
...
plot_range_ms = max([max(ts_send), max(ts_receive), max(ts_physync)]);
...
for i = 1:num_packets
    ...
    xStart = ts_send(i);       % Packet send time
    xEnd = ts_receive(i);     % Packet receive time
