...
load(savePath);
ts_dcilog = [dci_log.ts] - ts_pktOffset + [dci_log.k]/2 - time_drifting; 
...
tbs_threshold = median(tbs_physync_kbit) * 1.2; % Using median instead of hard-coded 14
...
for i = 1:size(hline_pkt, 1)
    ...
    xEnd = hline_pkt(i, 13);
    yPos = hline_pkt(i, 15); % Already normalized
...
xticks(0:25:125);
yticks(4:8:hline_ylim_upper);

set(gca, 'FontSize', 42);
set(gca, 'XTickLabel', []); % Remove x-axis labels
xlabel('Timestamp (ms)', 'FontSize', 42);
ylabel('Packet Index', 'FontSize', 42);
grid on;
hold off;
...
h2_filled = bar(ts_bsr(filled_bsr_idx), tbs_bsr(filled_bsr_idx), 'FaceColor', [0.4660 0.6740 0.1880], 'EdgeColor', [0.4660 0.6740 0.1880], 'LineWidth', 2, 'BarWidth', 0.04);
...
set(gca, 'FontSize', 42);
xlabel('Time (ms)', 'FontSize', 42);
ylabel('TBS (Kbits)', 'FontSize', 42);
grid on;
hold off;
