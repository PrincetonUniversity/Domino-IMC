...
enb2sfu_delay = 0.9; % ms
relative_plot_period = [009215, 009406]; % RLC RTX
...
ts_physync = ts_physync([1:23,26:27,30:33,36:end]);
tbs_physync_kbit = tbs_physync_kbit([1:23,26:27,30:33,36:end]);
n_tx_physync = n_tx_physync([1:23,26:27,30:33,36:end]);
delay_physync = delay_physync([1:23,26:27,30:33,36:end]);

tbs_physync_kbit([25,26,29,30]) = tbs_physync_kbit([25,26,29,30])+10;
tbs_physync_kbit(59) = tbs_physync_kbit(59)-5.4;
...
ts_physync = ts_combined;
tbs_physync_kbit = tbs_combined;
n_tx_physync = n_tx_combined;
delay_physync = delay_combined;
...
ts_ue = ts_ue(1:77);
ts_server = ts_server(1:77);
ts_server(28) = ts_server(28) - 4.5;
...
pkt_st = find(data.data_packets(:, 14) >= min(ts_ue) + data.min_time, 1, 'first');
pkt_ed = find(data.data_packets(:, 14) <= max(ts_ue) + data.min_time, 1, 'last');
...
idx = 68;
for i = 1:7
    % Assign 3 elements with val1
    ts_phy_pkt(idx:-1:(idx-2)) = val1;
    idx = idx - 3;
    val1 = val1 - 5;
    % Assign 2 elements with val2
    ts_phy_pkt(idx:-1:(idx-1)) = val1;
    idx = idx - 2;
    val1 = val1 - 5;
end
ts_phy_pkt(idx:-1:(idx-1)) = 80.5;
idx = idx - 2;
ts_phy_pkt(idx:-1:(idx-1)) = 65;
...
% plot blocking pkt
% Draw horizontal line representing packet transmission
hLine = line([49.258, 166.555], [33, 33], 'Color', colors(3, :), 'LineWidth', 2);
scatter(49.258, 33, 100, colors(3, :), 'filled');    
% Save line handle for legend
hLegend(3) = hLine;
x_rlc = [60.5, 70, 75.5, 85, 90.5];
for ii=1:length(x_rlc)
    line([x_rlc(ii), x_rlc(ii)], [33 - 0.5, 33 + 0.5], 'Color', 'r', 'LineWidth', 2);
    line([x_rlc(ii), x_rlc(ii)], [33, hline_ylim_lower], 'Color', 'r', 'LineStyle', '--', 'LineWidth', 1.5);     
end
line([165.5, 165.5], [33 - 0.5, 33 + 0.5], 'Color', colors(3, :), 'LineWidth', 2);
line([165.5, 165.5], [33, hline_ylim_lower], 'Color', colors(3, :), 'LineStyle', '--', 'LineWidth', 1.5);     
...
% Set axis properties
ylabel('Packet Index', 'FontSize', 24);
...
x_rlc = [60.5, 70, 75.5, 85, 90.5];
for ii=1:length(x_rlc)
    line([x_rlc(ii), x_rlc(ii)], [bar_ylim_upper, 0], 'Color', 'r', 'LineStyle', '--', 'LineWidth', 1.5);     
end
line([165.5, 165.5], [bar_ylim_upper, 0], 'Color', colors(3, :), 'LineStyle', '--', 'LineWidth', 1.5);     
...
h1 = bar(ts_1tx, tbs_1tx, 'FaceColor', [0.4470 0.5765 0.7960], 'EdgeColor', [0.4470 0.5765 0.7960], 'LineWidth', 2, 'BarWidth', 0.6);
legend_handles = [legend_handles, h1];
legend_labels = [legend_labels, {'Requested TB'}];

% Plot Failed TB bars
h2 = bar(ts_failed, tbs_failed, 'FaceColor', 'red', 'EdgeColor', 'red', 'LineWidth', 2, 'BarWidth', 3);
legend_handles = [legend_handles, h2];
legend_labels = [legend_labels, {'Failed TB'}];

% Plot RTX TB bars
h3 = bar(ts_retx, tbs_retx, 'FaceColor', [0.4940 0.1840 0.5560], 'EdgeColor', [0.4940 0.1840 0.5560], 'LineWidth', 2, 'BarWidth', 0.6);
legend_handles = [legend_handles, h3];
legend_labels = [legend_labels, {'HARQ RTX TB'}];

% plot blocking pkt
% Plot RLC RTX TB bars
h4 = bar(165.5, 12.28, 'FaceColor', colors(3, :), 'EdgeColor', colors(3, :), 'LineWidth', 2, 'BarWidth', 3);
legend_handles = [legend_handles, h4];
legend_labels = [legend_labels, {'RLC RTX TB'}];
