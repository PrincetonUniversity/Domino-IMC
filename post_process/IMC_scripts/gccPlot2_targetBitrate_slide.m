...
ylabel({'Pkt. One-Way'; 'Delay (ms)'}, 'FontSize', 12);
ylim(ylim_values);
yticks([0:150:450]);
xlim(xlim_values);
grid on;
...
if ~isempty(gcc_data)
    ...
    if ~isempty(trendline_data)
        ...
        text(1.75, 0, 'No trendline data available', 'HorizontalAlignment', 'center', 'FontSize', 12);
    end
else
    text(1.75, 0, 'GCC data not available', 'HorizontalAlignment', 'center', 'FontSize', 12);
end

% Formatting
ylabel({'Delay Variation'; 'Slope'}, 'FontSize', 12);
ylim(ylim_values);
xlim(xlim_values);
yticks([-60:30:60]);
grid on;
...
xlabel('Timestamp at Sender (s)', 'FontSize', 12);
