...
tbs_threshold = 0; % TBs smaller than this are considered proactive (kbits)
enb2sfu_delay = 1.1; % ms
relative_plot_period = [232174, 232300];
skip_init_tbs = 2;
...
i = 1;
while i <= length(ts_valid)
    current_ts = ts_valid(i);
    current_tbs = tbs_valid(i);
    
    % Look ahead to check for nearby TBs
    j = i + 1;
    while j <= length(ts_valid) && (ts_valid(j) - current_ts) <= 1
        % If within 1ms, combine the TBS and use the later timestamp
        current_tbs = current_tbs + tbs_valid(j);
        current_ts = ts_valid(i); % Use the later timestamp
        j = j + 1;
    end
...
% Replace the original arrays with combined ones
ts_valid = ts_combined(skip_init_tbs+1:end);
tbs_valid = tbs_combined(skip_init_tbs+1:end);

% Separate proactive and requested TBs
proact_idx = tbs_valid < tbs_threshold;
ts_proact = ts_valid(proact_idx);
tbs_proact = tbs_valid(proact_idx);

bsr_idx = tbs_valid >= tbs_threshold;
ts_bsr = ts_valid(bsr_idx);
tbs_bsr = tbs_valid(bsr_idx);
...
if length(pkt_indices) > 0
    ...
    hline_ylim_upper = max(pkt_indices - pkt_idx_adjust) + 4;
else
...
elseif numel(unique_streams) == 2
    lgd = legend(hLegend, {'Video Packets', 'Audio Packets'});
else
...
h2_filled = bar(ts_valid, tbs_valid, 'FaceColor', [0.4660 0.6740 0.1880], 'EdgeColor', [0.4660 0.6740 0.1880], 'LineWidth', 2, 'BarWidth', 0.6);
...
lgd = legend(legend_handles, legend_labels, 'NumColumns', 1);
lgd.FontSize = 26;
