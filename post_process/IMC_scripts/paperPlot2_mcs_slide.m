...
% grid on;
% % Set y-label with two rows
% ylabel({'Data Rate'; '(Mbps)'}, 'FontSize', 12);
% ylim(ylim_values);
% yticks([0, 2, 4, 6]);
% xlim([0, 3]);
...
grid on;
% Set y-label with two rows
ylabel({'Rate Gap'; '(Mbps)'}, 'FontSize', 12);
ylim(ylim_values);
yticks([-2, 0, 2, 4]);
xlim([0, 3]);
...
xlabel('Timestamp at Sender (s)', 'FontSize', 12);  % Keep x-label only for bottom subplot
ylabel({'Pkt. One-Way'; 'Delay (ms)'}, 'FontSize', 12);
ylim(ylim_values);
yticks([0:150:450]);
xlim([0, 3.0]);
grid on;
set(gca, 'FontSize', 12);  % Remove x-tick labels and set font size
hold off;
